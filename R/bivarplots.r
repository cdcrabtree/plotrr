...
bivarplots <- function(x, y, group, data) {
  ...
  for(i in 1:length(unique(data[[group]]))) {
    ...
    bivar.j <- ggplot2::ggplot(data[data[[group]]==j, ], aes(x = data[[x]][data[[group]]==j], y = data[[y]][data[[group]]==j])) + ggplot2::geom_jitter() + ggplot2::geom_smooth(method = loess) + ggplot2::labs(title = j) + xlab(x) + ylab(y)
